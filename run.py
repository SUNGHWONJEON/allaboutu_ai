...
def color_convert(img):
    ...
    # plt.imshow(img_avg)
    # plt.show()
    bgr_img_avg = cv2.cvtColor(img_avg, cv2.COLOR_RGB2BGR)

    # cv2.imwrite("img/" + file_name + "_9.jpg", bgr_img_avg)
    ...
    hsv_color = cv2.cvtColor(np.uint8([[float_tp_RGB_color]]), cv2.COLOR_BGR2HSV)[0][0]
    hsv_color[0] = round(hsv_color[0] * 359, 3)
    hsv_color[1] = round(hsv_color[1] * 100, 3) - 4
    hsv_color[2] = round(hsv_color[2] * 100, 3) + 8
    hsv_color = [round(hsv_color[0], 2), round(hsv_color[1], 2), round(hsv_color[2], 2)]

    return hsv_color, list_rgb_color
...
def upload_file(f):
    ...
    if np.array_equal(face_img, [0, 0, 0]):
        ...
        if success:
            ...
            print('run result 1 : ', result)


    print('run result 2 : ', result)
    print('run color_type : ', color_type[result])
